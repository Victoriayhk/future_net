...
void MinimumSpanningTree(int Sparse)
{
    ...
    Node *NextBlue = 0; /* Points to the provisional(临时的) next node to be included */
    Node *N;
    Candidate *NBlue;
    int d;
    ...
    if (Sparse && Blue->CandidateSet) //每次把2n-1个元素HeapLazyInsert,HeapDeleteMin
	{
        /* The graph is sparse */
        ...
        for (NBlue = Blue->CandidateSet; (N = NBlue->To); NBlue++)
		{
			...
			{
                N->Dad = Blue;
                N->Cost = NBlue->Cost + Blue->Pi + N->Pi;//NBlue->Cost = D(Blue, N)
                N->Rank = INT_MIN;
                HeapSiftUp(N);
            }
			else
        ...
        while ((NextBlue = HeapDeleteMin())) 
		{
            ...
            for (NBlue = Blue->CandidateSet; (N = NBlue->To); NBlue++) 
			{
				...
				else if ((d = NBlue->Cost + Blue->Pi + N->Pi) < N->Cost)
                {
                    N->Dad = Blue;
                    N->Cost = N->Rank = d;
                    HeapSiftUp(N);
	...
	else
	{
        ...
        while ((N = Blue->Suc) != FirstNode) 
		{
            ...
            do 
			{
				...
				{
                    if (!Forbidden(Blue, N) && (d = D(Blue, N)) < N->Cost) 
					{
                        N->Cost = d;
                        N->Dad = Blue;
                    }
