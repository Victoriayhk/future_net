...
Node *Best2OptMove(Node * t1, Node * t2, GainType * G0, GainType * Gain)
{
	...
	{
        if (t3 == t2->Pred || t3 == t2->Suc ||//不能是现在路径上的
            ((G1 = *G0 - Nt2->Cost) <= 0 && GainCriterionUsed))//X1-Y1>0
            continue;
        /* Choose t4 (only one choice gives a closed tour) */
        t4 = PRED(t3);//3前面的一个点
        if (Fixed(t3, t4))//t3,t4是fixed不能打破
            continue;
		
        G2 = G1 + C(t3, t4);
        if (!Forbidden(t4, t1) && (*Gain = G2 - C(t4, t1)) > 0) //t4-t1。此时可以进行2opt
		{
            Swap1(t1, t2, t3);
            return 0;
        }
